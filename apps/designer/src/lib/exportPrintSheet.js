...
export async function rasterizeCardToPng(args) {
    const svg = renderToStaticMarkup(React.createElement(CardRender, {
        template: args.template,
        data: args.data,
    ...
    const collectAsset = (layer) => {
        if (!evaluateApplies(layer.appliesWhen, visibleData))
            return null;
        if (parentVisibilityGated(layer, args.template.layers, visibleData))
            return null;
        const r = resolveLayer(layer, visibleData);
        if (r.type !== "image" && r.type !== "zone")
            return null;
        if (r.type === "image") {
    ...
    const urls = Array.from(new Set(args.template.layers
        .map(collectAsset)
        .filter((u) => typeof u === "string" && u.length > 0)));
    const dataUriByUrl = await Promise.all(urls.map(async (u) => [u, await fetchAsDataUri(u)]));
    let inlined = svg;
...
async function svgToPng(svgMarkup, w, h) {
