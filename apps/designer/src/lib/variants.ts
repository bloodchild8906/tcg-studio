...
export function evaluateApplies(
  rule: AppliesWhenRule | null | undefined,
 ...
 */
export function resolveLayer<L extends Layer>(
  layer: L,
  data: Record<string, unknown>,
): L {
  const variants = layer.variants;
  if (!variants || variants.length === 0) return layer;
  for (const v of variants) {
    if (matchesVariant(v, data)) {
      return { ...layer, ...(v.overrides as Partial<L>) } as L;
    }
  }
  return layer;
}
...
export function describeOperator(op: VariantOperator): string {
  return {
    equals: "equals",
    not_equals: "≠",
    in: "in",
