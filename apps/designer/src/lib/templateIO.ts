...
function assert(cond: unknown, msg: string, path?: string): asserts cond {
  if (!cond) throw new TemplateIOError(msg, path);
}
 ...
 */
export function parseTemplate(raw: unknown): CardTypeTemplate {
  ...
  raw.layers.forEach((layer, i) => {
    ...
    assert(
      l.type === "rect" || l.type === "text" || l.type === "image" || l.type === "zone",
      "layer.type must be one of rect|text|image|zone",
      `${path}.type`,
    );
  ...
  return raw as unknown as CardTypeTemplate;
}

/** Pretty-print a template for download. */
export function serializeTemplate(template: CardTypeTemplate): string {
 ...
 */
export function downloadTemplate(template: CardTypeTemplate): void {
...
export function pickTemplateFile(): Promise<CardTypeTemplate> {
  return new Promise((resolve, reject) => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        reject(
          err instanceof TemplateIOError
            ? err
            : new TemplateIOError(
                `failed to parse JSON: ${(err as Error).message ?? String(err)}`,
              ),
        );
      }
    };
